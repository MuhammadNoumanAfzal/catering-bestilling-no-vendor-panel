import { notificationMessage, translateNotificationText } from "../notificationTranslations";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 px-4 py-5">
      <div className="w-full max-w-[440px] rounded-[20px] border border-[#eee2d8] bg-white p-4 shadow-[0_20px_44px_rgba(0,0,0,0.22)] sm:p-4.5">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="m-0 text-[11px] font-bold uppercase tracking-[0.16em] text-[#d86f39]">{translateNotificationText(notification.detailTitle)}</p>
        ...
        <p className="mt-2.5 text-[14px] leading-6 text-[#6f6258]">{notificationMessage(notification)}</p>
            ...
            <div className="grid grid-cols-[minmax(108px,0.72fr)_1fr] gap-x-3 gap-y-2.5 max-[520px]:grid-cols-1">
              {rows.map((row) => (
                <Fragment key={`${notification.id}-${translateNotificationText(row.label)}`}>
                  ...
                  <span className="text-[13px] font-semibold leading-5 text-[#241c16]">{row.label === "Message" && notification.generatedOrder ? notificationMessage(notification) : ["Type", "Status", "Stage"].includes(row.label) ? translateNotificationText(row.value) : row.value}</span>
                </Fragment>
              ))}
            </div>
          )}
