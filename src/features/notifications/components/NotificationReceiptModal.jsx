    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 px-4 py-5">
      <div className="w-full max-w-[450px] rounded-[20px] border border-[#eee2d8] bg-white p-4 shadow-[0_20px_44px_rgba(0,0,0,0.22)]">
        <div className="flex items-start justify-between gap-3 border-b border-[#ede3db] pb-3">
          <div>
            <p className="m-0 text-[11px] font-bold uppercase tracking-[0.16em] text-[#d86f39]"> {translateNotificationText("Payment Receipt")} </p>
            <h2 className="mt-1 text-[22px] font-extrabold leading-[1.15] text-[#1c1510]">{translateNotificationText(notification.title)}</h2>
          </div>
          <button
        ...
        <div className="mt-3 rounded-[16px] bg-[#fff5ef] p-3">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="m-0 text-[11px] font-bold uppercase tracking-[0.12em] text-[#8e7f73]"> {translateNotificationText("Amount Received")} </p>
              <p className="mt-1 text-[22px] font-extrabold text-[#1d1611]">
                {amount || (isLoading ? translateNotificationText("Loading...") : "--")}
              </p>
            </div>
            <span className="rounded-full bg-white px-3 py-[6px] text-[11px] font-semibold text-[#2f8a4b]">
              {translateNotificationText(status)}
            </span>
          </div>
        </div>
        ...
        <div className="mt-3 grid grid-cols-[minmax(108px,0.72fr)_1fr] gap-x-3 gap-y-2.5 max-[520px]:grid-cols-1">
          {detailRows.map((row) => (
            <Fragment key={`${notification.id}-${translateNotificationText(row.label)}`}>
              <span className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#8b7d72]">{translateNotificationText(row.label)}</span>
              <span className="text-[13px] font-semibold leading-5 text-[#241c16]">{["Type", "Status", "Stage"].includes(row.label) ? translateNotificationText(row.value) : row.value}</span>
            </Fragment>
          ))}
        </div>
