...
export default function SupportTicketForm({ onSubmitted, initialForm = null }) {
  ...
  const {
    ...
    submitted,
    submittedTicketId,
  } = useSupportTicketForm(onSubmitted, initialForm);

  return (
    <section className="rounded-[18px] border border-[#e7ddd4] bg-white px-5 py-5 shadow-[0_10px_24px_rgba(43,30,20,0.05)]">
      <div>
        <h2 className="type-h4 m-0 text-[#181310]">{t("support.submitTitle", { defaultValue: "Submit a Support Ticket" })}</h2>
        <p className="type-para mt-1 text-[#8d8074]">
          {t("support.submitDescription", { defaultValue: "Share your issue and our team will get back to you with the right next steps." })}
        </p>
      </div>
      ...
      <form className="mt-4" onSubmit={handleSubmit}>
        <div className="grid gap-4">
          <div className="grid grid-cols-2 gap-3 max-[760px]:grid-cols-1">
            <SupportFieldSelect
              label={t("support.issueType", { defaultValue: "Subject Issue Type" })}
              onChange={handleFieldChange("issueType")}
              options={supportIssueTypeOptions}
              placeholder={t("support.selectIssue", { defaultValue: "Select issue type" })}
              value={form.issueType}
            />
            <label className="flex flex-col gap-1">
              <span className="text-[14px] font-bold text-[#2a211b]">
                {t("support.relatedOrder", { defaultValue: "Related Order" })} ({t("settings.optional", { defaultValue: "Optional" })})
              </span>
              <input
          ...
          <label className="flex flex-col gap-1">
            <span className="text-[14px] font-bold text-[#2a211b]">{t("support.description", { defaultValue: "Description" })}</span>
            <textarea
          ...
          <div>
            <p className="text-[14px] font-bold text-[#2a211b]">
              {t("support.attachment", { defaultValue: "Attachment" })} ({t("settings.optional", { defaultValue: "Optional" })})
            </p>
            <div className="mt-2">
          ...
          <div className="mt-4 flex items-center justify-between gap-3 max-[720px]:flex-col max-[720px]:items-stretch">
            ...
            <button
              ...
              type="submit"
            >
              {isSubmitting ? t("support.submitting", { defaultValue: "Submitting …" }) : t("support.submit", { defaultValue: "Submit Ticket" })}
            </button>
          </div>
        </div>
      </form>
